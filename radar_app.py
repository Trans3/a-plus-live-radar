...
# - v23 adds first-class Sharpshooter Mode for every non-expansion phase
#   with real-outlier scoring, fakeout penalties, phase-specific thresholds,
#   ranking, Discord/export fields, and scout-only sizing
#
# Notes:
...
# RSI trend-line intelligence (TradingView-style RSI + configurable MA overlay).
# Keep this diagnostic-first until proof analytics show that it improves entries.
RSI_MA_TYPE = os.getenv("RSI_MA_TYPE", "SMA").strip().upper()
RSI_MA_LENGTH = int(os.getenv("RSI_MA_LENGTH", "14"))
RSI_MA_SLOPE_LOOKBACK = int(os.getenv("RSI_MA_SLOPE_LOOKBACK", "3"))
...
# v23 Sharpshooter Mode: expansion uses normal continuation logic; every other
# phase requires an independently strong, well-confirmed outlier.
SHARPSHOOTER_PHASES = {"ACCUMULATION", "DISTRIBUTION", "EXHAUSTION", "CHOP", "BEAR", "WATCH"}
SHARPSHOOTER_MIN_SCORE = {
    "ACCUMULATION": 78,
    "DISTRIBUTION": 84,
    "EXHAUSTION": 88,
    "CHOP": 86,
...
SHARPSHOOTER_MIN_RELATIVE_STRENGTH = {
    "ACCUMULATION": 0.30,
    "DISTRIBUTION": 0.55,
    "EXHAUSTION": 0.75,
    "CHOP": 0.60,
...
def build_pre_aplus_message(
    ...
    return (
        ...
        f"• RSI-MA 1m: `{getattr(er, 'rsi_ma_1m', float('nan')):.2f}` | `{getattr(er, 'rsi_ma_state_1m', 'UNKNOWN')}` | gap `{getattr(er, 'rsi_ma_gap_1m', 0.0):+.2f}` | hold `{getattr(er, 'rsi_ma_hold_1m', 0)}`\n"
        f"• RSI-MA 5m: `{getattr(er, 'rsi_ma_5m', float('nan')):.2f}` | `{getattr(er, 'rsi_ma_state_5m', 'UNKNOWN')}` | score `{getattr(er, 'rsi_ma_score', 0):+d}`\n"
...
def build_discord_aplus_message(
    ...
    msg = (
        ...
        f"• Sharpshooter: `{getattr(er, 'sharpshooter_score', 0)}/{getattr(er, 'sharpshooter_threshold', 0)} | {getattr(er, 'outlier_label', 'N/A')} | Fakeout={getattr(er, 'fakeout_risk', 'UNKNOWN')}`\n"
        f"• Outlier proof: `{' | '.join((getattr(er, 'sharpshooter_reasons', []) or [])[:4]) or 'normal expansion logic'}`\n\n"
...
def content_signal_tag(er: "EvalResult", regime_name: str, item: Optional[dict] = None) -> str:
    if sharpshooter_mode_active(regime_name):
        return "SHARPSHOOTER" if getattr(er, "sharpshooter_qualified", False) else "WATCHLIST"
    if er.trade_ready:
        return "BULL"
    return "PREBUILD"
...
#
# Optional overrides:
#   $env:GITHUB_RADAR_REPO = "Trans3/a-plus-live-radar"
...
def normalize_github_repo(repo: str) -> str:
    ...
    repo = (repo or "Trans3/a-plus-live-radar").strip()
    if repo == "Trans3/a-plus-live-radar":
        return "Trans3/a-plus-live-radar"
    return repo
...
_cloud_auth_disabled = False
_cloud_auth_warning_printed = False


def _mark_cloud_auth_off(context: str, status_code: int, body: str = "") -> None:
    """Disable GitHub cloud sync after auth failure so scanner output stays clean."""
    global _cloud_auth_disabled, _cloud_auth_warning_printed
    _cloud_auth_disabled = True
    if not _cloud_auth_warning_printed:
        detail = (body or "").replace("\n", " ")[:160]
        print(f"[{ts()}] GitHub Cloud OFF: {context} returned HTTP {status_code}. Scanner will keep running locally. Restore GITHUB_RADAR_TOKEN later to re-enable sync. {detail}")
        _cloud_auth_warning_printed = True
...
def export_radar_state_json(
    ...
    for rank, item in enumerate((top_setups or [])[:10], start=1):
        ...
        setups.append({
            ...
            "rsi_ma_1m": round(_json_safe_number(getattr(er, "rsi_ma_1m", 0.0)), 3),
            "rsi_ma_5m": round(_json_safe_number(getattr(er, "rsi_ma_5m", 0.0)), 3),
            "rsi_ma_slope_1m": round(_json_safe_number(getattr(er, "rsi_ma_slope_1m", 0.0)), 3),
            "rsi_ma_slope_5m": round(_json_safe_number(getattr(er, "rsi_ma_slope_5m", 0.0)), 3),
            "rsi_ma_gap_1m": round(_json_safe_number(getattr(er, "rsi_ma_gap_1m", 0.0)), 3),
            "rsi_ma_gap_5m": round(_json_safe_number(getattr(er, "rsi_ma_gap_5m", 0.0)), 3),
            "rsi_ma_hold_1m": int(getattr(er, "rsi_ma_hold_1m", 0)),
            "rsi_ma_hold_5m": int(getattr(er, "rsi_ma_hold_5m", 0)),
            "rsi_ma_state_1m": getattr(er, "rsi_ma_state_1m", "UNKNOWN"),
            "rsi_ma_state_5m": getattr(er, "rsi_ma_state_5m", "UNKNOWN"),
            "rsi_ma_score": int(getattr(er, "rsi_ma_score", 0)),
            ...
            "sharpshooter_mode": sharpshooter_mode_active(regime_name),
            "sharpshooter_score": int(getattr(er, "sharpshooter_score", 0)),
            "sharpshooter_threshold": int(getattr(er, "sharpshooter_threshold", 0)),
            "sharpshooter_qualified": bool(getattr(er, "sharpshooter_qualified", False)),
            "outlier_label": getattr(er, "outlier_label", "N/A"),
            "fakeout_risk": getattr(er, "fakeout_risk", "UNKNOWN"),
            "sharpshooter_reasons": list(getattr(er, "sharpshooter_reasons", []) or []),
            "sharpshooter_penalties": list(getattr(er, "sharpshooter_penalties", []) or []),
...
def push_radar_state_to_github(path: str = RADAR_STATE_JSON_PATH, force: bool = False) -> bool:
    ...
    if not CLOUD_PUSH_ENABLED or _cloud_auth_disabled:
        return False
    ...
    try:
        ...
        if get_resp.status_code == 200:
            ...
            _mark_cloud_auth_off("radar_state lookup", get_resp.status_code, get_resp.text)
            return False
    ...
    try:
        ...
        if put_resp.status_code in (401, 403):
            _mark_cloud_auth_off("radar_state push", put_resp.status_code, put_resp.text)
            return False
...
def push_performance_to_github(path: str = PERFORMANCE_JSON_PATH, force: bool = False) -> bool:
    ...
    try:
        ...
        if get_resp.status_code == 200:
            ...
            _mark_cloud_auth_off("performance lookup", get_resp.status_code, get_resp.text)
            return False
        elif get_resp.status_code not in (404,):
    ...
    try:
        ...
        if 200 <= put_resp.status_code < 300:
            ...
            return True
        if put_resp.status_code in (401, 403):
            _mark_cloud_auth_off("performance push", put_resp.status_code, put_resp.text)
            return False
...
def rsi_series(series: np.ndarray, period: int = 14) -> np.ndarray:
    """Return a full RSI series while preserving Radar's existing RSI math.

    Radar historically calculates RSI from the most recent `period` gains/losses
    using simple averages. Building the series the same way keeps existing scanner
    thresholds stable while giving us enough history to calculate an RSI MA,
    crosses, slope, separation, and hold duration.
    """
...
def moving_average_series(series: np.ndarray, length: int, ma_type: str = "SMA") -> np.ndarray:
    """Moving average over a series that may begin with NaNs (as RSI does)."""
    if series is None or length <= 0:
        return np.array([], dtype=float)
    arr = np.asarray(series, dtype=float)
    out = np.full(len(arr), np.nan, dtype=float)
    ...
    if ma_type == "EMA":
        ...
        seed = float(np.mean(vals[:length]))
        pos = start + length - 1
        out[pos] = seed
        prev = seed
        for j in range(length, len(vals)):
            prev = alpha * float(vals[j]) + (1.0 - alpha) * prev
            out[start + j] = prev
        ...
        for j in range(length - 1, len(vals)):
            out[start + j] = float(np.dot(vals[j - length + 1:j + 1], weights) / denom)
    else:  # SMA default
        for j in range(length - 1, len(vals)):
            out[start + j] = float(np.mean(vals[j - length + 1:j + 1]))
    return out


def rsi_ma_state(close_series: np.ndarray, period: int = RSI_PERIOD) -> dict:
    """Describe RSI vs its trend-line MA for one timeframe."""
    rs = rsi_series(close_series, period)
    if len(rs) == 0:
        return {
            "rsi": float("nan"), "ma": float("nan"), "above": False,
            "cross_up": False, "cross_down": False, "ma_slope": 0.0,
            "gap": 0.0, "hold_bars": 0, "state": "UNKNOWN",
        }
    ma = moving_average_series(rs, RSI_MA_LENGTH, RSI_MA_TYPE)
    finite = np.where(np.isfinite(rs) & np.isfinite(ma))[0]
    if len(finite) == 0:
        return {
            "rsi": float(rs[-1]), "ma": float("nan"), "above": False,
            "cross_up": False, "cross_down": False, "ma_slope": 0.0,
            "gap": 0.0, "hold_bars": 0, "state": "WARMING_UP",
        }
    ...
    if len(finite) >= 2:
        p = int(finite[-2])
        cross_up = bool(rs[p] <= ma[p] and r_now > m_now)
        cross_down = bool(rs[p] >= ma[p] and r_now < m_now)

    lb = max(1, RSI_MA_SLOPE_LOOKBACK)
    ma_slope = 0.0
    if len(finite) > lb:
        j = int(finite[-(lb + 1)])
        ma_slope = m_now - float(ma[j])

    hold_bars = 0
    for j in reversed(finite):
        same_side = (rs[j] > ma[j]) if above else (rs[j] < ma[j])
        if not same_side:
            break
        hold_bars += 1
    ...
    if cross_up:
        ...
        state = "CROSS_DOWN"
    elif above and ma_slope > 0:
        state = "BULLISH_RISING"
    elif above:
        state = "ABOVE_MA"
    elif (not above) and ma_slope < 0:
        state = "BEARISH_FALLING"
    else:
        state = "BELOW_MA"

    return {
        "rsi": r_now, "ma": m_now, "above": bool(above),
        "cross_up": bool(cross_up), "cross_down": bool(cross_down),
        "ma_slope": float(ma_slope), "gap": float(gap),
        "hold_bars": int(hold_bars), "state": state,
    }
...
# =========================
# v23 SHARPSHOOTER / REAL-OUTLIER ENGINE
# =========================

def sharpshooter_mode_active(regime_name: str) -> bool:
    return (regime_name or "CHOP").upper() != "EXPANSION"


def compute_sharpshooter_score(
    er,
    regime_name: str,
    sector_count: int = 0,
    sector_strong: bool = False,
    ...
    """Score whether a non-expansion mover is a real outlier or a likely fake spike.

    The score deliberately rewards independent strength plus confirmation, while
    punishing unsupported, extended, low-participation moves. It is only the
    primary execution score outside EXPANSION.
    """
    score = 0
    reasons = []
    penalties = []
    rs = safe_float(getattr(er, "relative_strength_pct", 0.0))
    dist = abs(safe_float(getattr(er, "distance_from_vwap_pct", 999.0)))
    flags = getattr(er, "flags", {}) or {}

    # Independent strength vs BTC: 25 points.
    if rs >= 1.50:
        score += 25; reasons.append("elite relative strength")
    elif rs >= 0.90:
        score += 21; reasons.append("strong relative strength")
    elif rs >= 0.55:
        score += 17; reasons.append("clear BTC outperformance")
    elif rs >= 0.30:
        score += 11; reasons.append("positive relative strength")
    elif rs > 0:
        score += 5
    else:
        penalties.append("not outperforming BTC")

    # Sector confirmation: 20 points, but a solo leader can still qualify.
    if sector_strong:
        score += 20; reasons.append("sector confirming")
    elif sector_count >= 3:
        score += 14; reasons.append("sector participation")
    elif sector_count >= 2:
        score += 9; reasons.append("partial sector support")
    elif rs >= 1.0:
        score += 5; reasons.append("solo leader with elite RS")
    else:
        penalties.append("weak sector support")

    if flags.get("volume_spike", False):
        score += 13; reasons.append("volume participation")
    else:
        score -= 10; penalties.append("no volume expansion")

    if flags.get("vwap_accept", False):
        score += 12; reasons.append("VWAP accepted")
    else:
        score -= 18; penalties.append("VWAP not accepted")

    if flags.get("pullback", False):
        score += 9; reasons.append("pullback held")
    elif dist > 1.50:
        score -= 12; penalties.append("extended without pullback")

    if flags.get("structure_break", False):
        score += 8; reasons.append("structure break")
    else:
        score -= 5; penalties.append("no structure break")

    if flags.get("acceleration", False):
        score += 6; reasons.append("momentum accelerating")
    elif flags.get("impulse", False):
        score += 3

    mq = str(getattr(er, "momentum_quality", "UNKNOWN")).upper()
    if mq == "BUILDING":
        score += 7; reasons.append("momentum building")
    elif mq == "STABLE":
        score += 4
    elif mq == "WEAKENING":
        score -= 10; penalties.append("momentum weakening")
    elif mq == "FAILING":
        score -= 25; penalties.append("momentum failing")

    trap = str(getattr(er, "trap_risk", "LOW")).upper()
    if trap == "MEDIUM":
        score -= 12; penalties.append("medium trap risk")
    elif trap == "HIGH":
        score -= 30; penalties.append("high trap risk")

    if fresh_breakout:
        score += 5; reasons.append("fresh breakout")

    # Location discipline: a sharpshooter cannot chase a vertical anomaly.
    if dist <= 0.75:
        score += 5; reasons.append("defined risk near VWAP")
    elif dist > 2.25:
        score -= 15; penalties.append("far from value")

    score = max(0, min(100, int(round(score))))
    phase = (regime_name or "CHOP").upper()
    threshold = SHARPSHOOTER_MIN_SCORE.get(phase, 86)
    min_rs = SHARPSHOOTER_MIN_RELATIVE_STRENGTH.get(phase, 0.60)

    hard_gates = (
        rs >= min_rs
        and flags.get("vwap_accept", False)
        and flags.get("volume_spike", False)
        and flags.get("structure_break", False)
        and trap != "HIGH"
        and mq in {"BUILDING", "STABLE"}
        and dist <= 2.25
    )
    qualified = score >= threshold and hard_gates
    label = "REAL OUTLIER" if qualified else ("WATCH OUTLIER" if score >= threshold - 10 else "FAKEOUT RISK")
    fakeout_risk = "LOW" if qualified else ("MEDIUM" if score >= threshold - 10 else "HIGH")
...
class EvalResult:
    ...
    rsi_ma_1m: float = float("nan")
    rsi_ma_5m: float = float("nan")
    rsi_ma_slope_1m: float = 0.0
    rsi_ma_slope_5m: float = 0.0
    rsi_ma_gap_1m: float = 0.0
    rsi_ma_gap_5m: float = 0.0
    rsi_ma_hold_1m: int = 0
    rsi_ma_hold_5m: int = 0
    rsi_ma_state_1m: str = "UNKNOWN"
    rsi_ma_state_5m: str = "UNKNOWN"
    rsi_ma_score: int = 0
    sharpshooter_score: int = 0
    sharpshooter_threshold: int = 0
    sharpshooter_qualified: bool = False
    outlier_label: str = "N/A"
    fakeout_risk: str = "UNKNOWN"
    sharpshooter_reasons: Optional[List[str]] = None
    sharpshooter_penalties: Optional[List[str]] = None
...
def eval_pair(
    ...
    rsi1_state = rsi_ma_state(close1, RSI_PERIOD)
    rsi5_state = rsi_ma_state(close5, RSI_PERIOD)
    rsi1 = rsi1_state["rsi"]
    rsi1_prev = rsi(close1[:-3], RSI_PERIOD) if len(close1) > 20 else float("nan")
    rsi5 = rsi5_state["rsi"]
    rsi5_prev = rsi(close5[:-2], RSI_PERIOD) if len(close5) > 20 else float("nan")

    # Diagnostic momentum score only for now; it does NOT gate trades or alter
    # trigger_score. This lets proof analytics validate the signal first.
    rsi_ma_score = 0
    rsi_ma_score += 3 if rsi1_state["above"] else -3
    rsi_ma_score += 3 if rsi1_state["ma_slope"] > 0 else -2 if rsi1_state["ma_slope"] < 0 else 0
    rsi_ma_score += 4 if rsi1_state["cross_up"] else -4 if rsi1_state["cross_down"] else 0
    rsi_ma_score += 2 if rsi5_state["above"] else -2
    rsi_ma_score += 2 if rsi5_state["ma_slope"] > 0 else -1 if rsi5_state["ma_slope"] < 0 else 0
    rsi_ma_score = max(-12, min(14, int(rsi_ma_score)))
    ...
    flags = {
        ...
        "rsi_above_ma_1m": bool(rsi1_state["above"]),
        "rsi_cross_up_1m": bool(rsi1_state["cross_up"]),
        "rsi_cross_down_1m": bool(rsi1_state["cross_down"]),
        "rsi_ma_rising_1m": bool(rsi1_state["ma_slope"] > 0),
        "rsi_above_ma_5m": bool(rsi5_state["above"]),
        "rsi_cross_up_5m": bool(rsi5_state["cross_up"]),
        "rsi_cross_down_5m": bool(rsi5_state["cross_down"]),
        "rsi_ma_rising_5m": bool(rsi5_state["ma_slope"] > 0),
    }
    ...
    return EvalResult(
        ...
        decision_reason="",
        rsi_ma_1m=float(rsi1_state["ma"]),
        rsi_ma_5m=float(rsi5_state["ma"]),
        rsi_ma_slope_1m=float(rsi1_state["ma_slope"]),
        rsi_ma_slope_5m=float(rsi5_state["ma_slope"]),
        rsi_ma_gap_1m=float(rsi1_state["gap"]),
        rsi_ma_gap_5m=float(rsi5_state["gap"]),
        rsi_ma_hold_1m=int(rsi1_state["hold_bars"]),
        rsi_ma_hold_5m=int(rsi5_state["hold_bars"]),
        rsi_ma_state_1m=str(rsi1_state["state"]),
        rsi_ma_state_5m=str(rsi5_state["state"]),
        rsi_ma_score=int(rsi_ma_score),
    )
    ...
    try:
        with open("scanner_log.txt", "a", encoding="utf-8") as f:
            f.write(
                ...
                f"RSI1={er.rsi_1m:.2f} | "
                f"RSI5={er.rsi_5m:.2f} | "
                f"RSIMA1={getattr(er, 'rsi_ma_1m', float('nan')):.2f} | "
                f"RSIMA1STATE={getattr(er, 'rsi_ma_state_1m', 'UNKNOWN')} | "
                f"RSIMA1GAP={getattr(er, 'rsi_ma_gap_1m', 0.0):+.2f} | "
                f"RSIMA5={getattr(er, 'rsi_ma_5m', float('nan')):.2f} | "
                f"RSIMA5STATE={getattr(er, 'rsi_ma_state_5m', 'UNKNOWN')} | "
                f"RSIMASCORE={getattr(er, 'rsi_ma_score', 0):+d} | "
    ...
    while True:
        ...
        if watch_candidates:
            ...
            for rank, item in enumerate(full_watch, start=1):
                ...
                ss_result = compute_sharpshooter_score(
                    er,
                    regime_name=regime_name,
                    sector_count=item["sector_count"],
                    sector_strong=item["sector_strong"],
                ...
                er.sharpshooter_score = ss_result["score"]
                er.sharpshooter_threshold = ss_result["threshold"]
                er.sharpshooter_qualified = ss_result["qualified"]
                er.outlier_label = ss_result["label"]
                er.fakeout_risk = ss_result["fakeout_risk"]
                er.sharpshooter_reasons = ss_result["reasons"]
                er.sharpshooter_penalties = ss_result["penalties"]
                item["sharpshooter_score"] = ss_result["score"]
                item["sharpshooter_qualified"] = ss_result["qualified"]

                sharpshooter_bypass = (
                    sharpshooter_mode_active(regime_name)
                    and er.sharpshooter_qualified
                ...
                if sharpshooter_mode_active(regime_name):
                    er.position_size = "SCOUT" if premium_candidate else "NO TRADE"
                    er.veteran_decision = "SHARPSHOOTER" if premium_candidate else er.veteran_decision
                current_ranks[er.wsname] = rank

            # V23: Expansion ranks continuation quality. Every other phase ranks
            # real-outlier / Sharpshooter quality first to suppress fake leaders.
            full_watch = sorted(
                full_watch,
                key=lambda x: (
                    x.get("environment_score", 0) if not sharpshooter_mode_active(regime_name) else x.get("sharpshooter_score", 0),
        ...
        try:
            ...
            if export_ok:
                ...
                cloud_label = "OK" if cloud_ok else ("OFFLINE" if _cloud_auth_disabled else "SKIPPED")
                print(f"[{ts()}] Cloud push status: {cloud_label}")
                ...
                if perf_ok:
                    ...
                    perf_cloud_label = "OK" if perf_cloud_ok else ("OFFLINE" if _cloud_auth_disabled else "SKIPPED")
                    print(f"[{ts()}] Performance cloud push status: {perf_cloud_label}")
        except Exception as e:
